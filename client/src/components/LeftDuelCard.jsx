    ...
    <form onSubmit={handleSubmit} className="flex items-center gap-2">
      <div className="relative">
        <input
          ...
          placeholder="0.00"
          className="w-32 bg-[#0c0f16] border-2 border-cyan-400 rounded-lg pl-8 pr-2 py-2 text-white focus:outline-none focus:ring-2 focus:ring-cyan-300"
        />
        <FaEthereum className="absolute left-2 top-1/2 -translate-y-1/2 text-cyan-400" />
      </div>
      <button type="submit" className="px-4 py-2 rounded-lg font-bold text-lg bg-cyan-400 text-black transition-transform duration-200 hover:scale-105">
        Place Bid
      </button>
    </form>
  );
...
export default function LeftDuelCard({ meme, onBid, totalBids }) {
  // Ensure meme object exists to prevent errors
  if (!meme) return null;

  return (
    <div className={`flex flex-col bg-[#181824] border-2 border-cyan-400 rounded-2xl w-full max-w-lg mx-auto shadow-lg relative transition-all duration-300`}>
      {/* Fake window bar */}
      ...
      <div className="border-t border-[#222] bg-[#101014] px-6 py-4">
        <div className="text-sm text-gray-300 mb-2 space-y-2">
          <div className="flex items-center gap-2 text-lg"><FiTag className="text-cyan-400" /> <span className="text-gray-400">Title:</span> <span className="text-white">{meme.title}</span></div>
          <div className="flex items-center gap-2 text-lg"><FiUser className="text-cyan-400" /> <span className="text-gray-400">Creator:</span> <span className="text-white">{meme.creator.username}</span></div>
      ...
      <div className="flex items-center justify-between bg-[#101014] rounded-b-2xl px-6 py-4 border-t border-[#222]">
        ...
        <div className="text-cyan-400 text-2xl font-extrabold flex items-center gap-2">
